...
export class AssetsGateway implements OnGatewayInit {
  ...
  constructor(private assetsService: AssetsService, private assetDailiesService: AssetDailiesService) { }

  afterInit(server: Server) {
    this.assetsService.subscribeNewPriceUpdatedEvents().subscribe((asset) => {
      server.to(asset.symbol).emit('assets/priceUpdated', new AssetPresenter(asset).toJSON());
    });

    this.assetDailiesService.subscribeCreatedEvents().subscribe((assetDaily) => {
      server.to(assetDaily.asset.symbol).emit('assets/dailyCreated', new AssetDailyPresenter(assetDaily).toJSON());
    });
  }
